...
class ReasoningRouter(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            hrm_forward_fn: processes states through HRM -> [B, L, H]
            direct_head_fn: processes states directly -> [B, L, H]
            force_hrm: if True, bypass logic and route ALL through HRM
        Returns:
            output: [B, L, H] processed hidden states
            route_mask: [B] routing decisions (True = HRM)
        """
        if force_hrm:
