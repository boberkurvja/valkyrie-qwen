...
class HRMTrainConfig:
    ...
    num_examples: int = 2500     # halved; compensated by longer seq_len
    seq_len: int = 512           # doubled — more signal per step
    # Training
    batch_size: int = 2         # doubled (2 GPUs × 2 per GPU)
    learning_rate: float = 5e-5  # increased from 3e-5
    warmup_steps: int = 100      # shorter warmup
    num_epochs: int = 25         
    max_steps: int = 75_000      # hard cap to prevent late-stage overfitting
    # ACT config
    M_max: int = 16
    M_min: int = 2
    ...
    beta2: float = 0.95
    # Checkpointing
    save_steps: int = 2500       # was 5000 — more frequent checkpoints/uploads
    eval_steps: int = 100
