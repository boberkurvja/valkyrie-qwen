...
class ValkyrieGenerator:
    ...
    def generate(
        ...
        # Sticky routing: if any token in prompt is <|reason|>, or if use_hrm is requested
        # we will activate HRM for the entire reasoning block.
        reason_id = self.model.router.reason_token_id
        is_reasoning = (input_ids == reason_id).any().item() if reason_id is not None else False

        for step in range(max_new_tokens):
            # 1. Forward pass (leverages model-native routing)
            # If we are in reasoning mode, we FORCE HRM for every generated token
            # to ensure the coprocessor maintains coherence across the sequence.
            out = self.model(
                input_ids=current_token,
                past_key_values=sink_cache,
                use_cache=True,
                force_hrm=(use_hrm or is_reasoning)
            )

            
            # 2. Logits from the model (already through StableMax)
            ...
            # 4. Update 'sticky' reasoning state if we just generated the reason token
            if not is_reasoning and reason_id is not None:
                if (next_token == reason_id).any():
                    is_reasoning = True
            ...
            # Stream output
            if stream:
                # Extract the raw integer item to prevent nested list decoding
